...
export class ShopManager {
     ...
     */
    create(guildID: string, item: EconomyGuildShopItem): Promise<EconomyGuildShopItem> {
     ...
     */
    update<
        K extends keyof EconomyGuildShopItem
    >(guildID: string, itemID: number, key: K, value: EconomyGuildShopItem[K]): Promise<boolean | EconomyGuildShopItem> {
        return new Promise(async (res, rej) => {
            var data = await this.database.get(guildID);
            if(!data) data = await this.database.createGuild(guildID);
