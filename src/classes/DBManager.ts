...
export class DBManager {
    ...
    constructor(options: Options) {
        ...
        this.database = new Enmap({
            ...
            dataDir: this.options.DBPath !== undefined ? this.options.DBPath : undefined
        });
    }
     ...
     */
    async get(id: string): Promise<EconomyGuildData> {
     ...
     * @returns {Promise<boolean>}
     */
    async setProp(guildID: string, userID: string, key: string, value: number): Promise<boolean> {
        return new Promise(async (res, rej) => {
     ...
     * @returns {Promise<boolean>}
     */
    async push(id: string, value: any): Promise<boolean> {
        return new Promise((res, rej) => {
     ...
     * @returns {Promise<boolean>}
     */
    async add(guildID: string, userID: string, key: string, value: number): Promise<boolean> {
        return new Promise(async (res, rej) => {
            var data = this.database.fetch(`economy-${guildID}`);
            if(!data) data = await this.createGuild(guildID);
     ...
     * @returns {Promise<boolean>}
     */
    async subtract(guildID: string, userID: string, key: string, value: number): Promise<boolean> {
        return new Promise(async (res, rej) => {
            var data = this.database.fetch(`economy-${guildID}`);
            if(!data) data = await this.createGuild(guildID);
