...
export class ItemsManager {
     ...
     */
    buy(guildID: string, userID: string, itemID: number): Promise<boolean|ErrorObject> {
        return new Promise(async (res, rej) => {
            var data = await this.database.get(guildID);

            if(!data) data = await this.database.createGuild(guildID);
            if(!data.shop.length) return res({
                status: false,
            ...
            if(!user.inventory.length) return res({
                status: false,
            ...
            if(user['balance'] < item['cost']) return res({
                status: false,
     ...
     * @returns {Promise<boolean|ErrorObject>} 
     */
    sell(guildID: string, userID: string, itemID: number): Promise<boolean|ErrorObject> {
        return new Promise(async (res, rej) => {
            var data = await this.database.get(guildID);
     ...
     */
    use(guildID: string, userID: string, itemID: number): Promise<EconomyGuildShopItem|ErrorObject> {
        return new Promise(async (res, rej) => {
            var data = await this.database.get(guildID);
     ...
     * @returns {Promise<EconomyGuildShopItem|ErrorObject>} 
     */
    get(guildID: string, userID: string, itemID: number): Promise<EconomyGuildShopItem|ErrorObject> {
        return new Promise(async (res, rej) => {
            var data = await this.database.get(guildID);
     ...
     * @returns {Promise<EconomyGuildShopItem[]|ErrorObject>} 
     */
    all(guildID: string, userID: string): Promise<EconomyGuildShopItem[]|ErrorObject> {
        return new Promise(async (res, rej) => {
            var data = await this.database.get(guildID);
